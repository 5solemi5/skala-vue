...
// reactive로 선언한 배열 (날씨 앱의 즐겨찾기 도시 리스트 모형)
...
watch(cityList, (newArr, oldArr) => {
  // newArr.length와 oldArr.length가 똑같이 늘어난 상태로 출력된다.
...
watch(
  () => [...cityList],
  (newArr, oldArr) => {
    // 🔥 구조 분해 복사본을 감시하므로 과거 배열의 원본 데이터와 길이가 그대로 살아있다.
...
<template>
  <div class="practice-section">
    ...
    <div class="monitor auto">
      ...
      <p>{{ logAuto }}</p>
      <small>※ 주의: 이전 배열과 현재 배열의 내용물/길이가 똑같이 동기화되어 버린다.</small>
    </div>

    <div class="monitor target">
      <h3>🎯 2) () =&gt; [...cityList] 복사본 감시</h3>
      <p>{{ logCopy }}</p>
      <small>※ 성공: 과거 배열에 들어있던 내용물이 지워지지 않고 정상 대조된다.</small>
    </div>
  </div>
</template>
