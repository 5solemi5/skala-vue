...
const troubleList = ref([
  {
    ...
    date: '4일차',
    symptom:
      '/weather/city_05 에서 /weather/city_99 로 주소를 바꿨는데 화면에는 계속 대구가 떠 있었다. 없는 도시 코드인데도 안내 문구가 안 나왔다.',
    log: `onMounted(() => {
  ...
  if (mockDetails[id]) {
    ...
    cause:
      'Vue Router 는 같은 컴포넌트로 매칭되는 경로 사이를 이동할 때 컴포넌트를 파괴하지 않고 재사용한다. 파라미터만 바뀌는 경우가 여기 해당해서 onMounted 가 다시 실행되지 않는다.',
    fix: `watch(
  () => route.params.cityId,
  (newId) => { if (newId) loadCity(newId) },
)`,
    result:
      'city_01 → city_04 → city_06 → city_99 → city_02 로 연속 이동해도 매번 올바른 도시가 표시되고, 없는 코드에서는 안내 문구가 나온다.',
    lesson:
      "교재는 'Mount 시점에 Mock Data 에서 도시 객체 선택' 이라고 되어 있어 onMounted 만 썼는데, 동적 경로에서는 그것만으로 부족했다. 데이터 로딩을 함수로 빼서 onMounted 와 watch 양쪽에서 부르도록 했다.",
  },
  {
    id: 't1',
    ...
    date: '1일차',
    symptom:
      '프로젝트를 받아 처음 실행했을 때 vite 명령을 찾지 못했다. node_modules 는 분명히 있었다.',
...
Cannot find module '@rolldown/binding-darwin-arm64'
  ... library load disallowed by system policy`,
    ...
    cause:
      'npm 의 optional dependency 설치 이슈. 에러 메시지 자체가 npm/cli#4828 을 지목하고 있었다.',
    fix: 'rm -rf node_modules && npm ci',
    result: '16MB 짜리 rolldown-binding.darwin-arm64.node 가 정상적으로 받아졌고 개발 서버가 떴다.',
    lesson:
      'npm install 대신 npm ci 를 쓴 이유는 lock 파일에 적힌 버전 그대로 설치해서 강사님 환경과 어긋나지 않게 하기 위해서였다.',
  },
  {
    id: 't2',
    ...
    date: '1일차',
    symptom:
      '카드 그리드를 만들었는데 화면이 넓어져도 카드가 한 줄에 하나씩만 들어가고, 페이지 전체가 세로 가운데로 몰렸다.',
...
@media (min-width: 1024px) {
  ...
  #app { display: grid; grid-template-columns: 1fr 1fr; }
    ...
    cause:
      'npm create vue 로 만든 스캐폴드의 main.css 에 들어있는 웰컴 화면 전용 레이아웃. 1024px 이상에서 #app 을 2단 그리드로 쪼개고 body 를 flex 중앙정렬한다.',
    fix: '해당 미디어쿼리에서 2단 그리드와 body 중앙정렬을 제거하고, 왜 지웠는지 주석으로 남겼다.',
    result: '카드 그리드가 화면 폭에 맞춰 정상적으로 펼쳐졌다.',
    lesson:
      '내가 안 쓴 스타일이 먹고 있으면 프레임워크 기본값을 의심해야 한다. 스캐폴드 CSS 는 데모용이라 실제 레이아웃과 충돌할 수 있다.',
  },
  {
    id: 't3',
    ...
    date: '1일차',
    symptom:
      '교재 예제를 따라 .number 수식어 실습을 하는데 숫자를 입력해도 데이터 타입이 계속 string 으로 나왔다.',
...
<input type="text" v-model="age" />`,
    ...
    result:
      '입력값 25 의 typeof 가 number 로 바뀌었고, "   test@mail.com   " 의 길이가 13 으로 나와 공백 제거가 확인됐다.',
    lesson:
      '예제가 안 되면 내 문법을 의심하기 전에 예제 자체를 먼저 읽어봐야 한다. 화면에 결과를 찍어두면 이런 걸 빨리 잡을 수 있다.',
  },
])
...
<div v-for="advice in getAdviceList(item)">`,
  ...
  return map
    ...
    effect:
      'computed 안에 로그를 넣고 확인해 보니, 모드를 바꿀 때만 재계산되고 카드를 클릭할 때는 캐싱된 값이 재사용됐다.',
...
<template>
  <div class="devlog">
    <h2>🛠️ 개발 기록</h2>
    <p class="lead">
      과제를 만들면서 실제로 막혔던 문제와, 배운 것을 적용해 고쳐 쓴 코드를 남겨둡니다. 완성된
      결과만으로는 어떤 판단을 거쳤는지 남지 않기 때문입니다.
    </p>
