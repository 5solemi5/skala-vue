...
const troubleList = ref([
  {
    id: 't14',
    title: '마당에서 나간 사람이 지워지지 않고 쌓임',
    date: '내 사람들 마당',
    symptom:
      '카드를 지우면 표에서는 사라지는데 마당에는 그대로 남았다. 지우고 되돌리기를 반복하니 네 명짜리 목록인데 마당에는 열여섯이 걸어다녔다.',
    log: `// 카드 수와 마당 인원을 같이 세어 봤다
카드 12  마당 12
카드 11  마당 12   <- 여기서 벌어진다
카드 10  마당 12
카드  4  마당 16   <- 되돌리자 오히려 늘었다`,
    cause:
      'TransitionGroup 은 나가는 요소를 바로 지우지 않고 끝날 때까지 기다린다. 무엇을 기다릴지는 그 요소의 스타일을 보고 스스로 고르는데, 캐릭터에는 걷는 animation 과 사라지는 transition 이 둘 다 걸려 있었다. animation 쪽을 골라 animationend 를 기다렸는데 걸음은 infinite 라 그 순간이 오지 않았다.',
    fix: `<!-- 무엇을 기다릴지 직접 정해 준다 -->
<TransitionGroup name="walker" type="transition">`,
    note: '무한 반복 애니메이션이 걸린 요소에 나가는 효과를 주려면 type 을 정해 줘야 한다. 화면에서는 사라진 것처럼 보여서(투명도 0) 한참 못 찾았다.',
  },
  {
    id: 't13',
    title: '열두 명이 거의 같은 옷을 입고 나옴',
    date: '내 사람들 마당',
    symptom:
      '사람마다 옷 색을 여섯 가지 중에서 고르게 했는데, 열두 명 중 열 명이 같은 색이었다. 앞뒤 자리도 두 종류밖에 안 나왔다.',
    log: `// 실제로 뽑힌 값을 찍어 봤다
옷색: ['#a8763a','#a8763a','#a8763a','#a8763a','#a8763a',
       '#a8763a','#a8763a','#a8763a','#a8763a','#a8763a',
       '#3f6f63','#3f6f63']
앞뒤: ['7px','7px','7px','7px','7px','7px','7px','7px','7px','7px','25px','25px']`,
    cause:
      "id 가 p_seed0 ~ p_seed11 처럼 끝 글자만 다르다. 곱하고 더하는 해시라 끝 글자가 만드는 차이는 하위 비트에만 남는데, 값을 뽑을 때 (seed >>> 19) 처럼 상위 비트를 보고 있었다. 상위 비트는 열두 명이 거의 같았다.",
    fix: `// 마지막에 한 번 섞어 하위 비트의 차이를 위로 퍼뜨린다
n ^= n >>> 16
n = Math.imul(n, 2246822507) >>> 0
n ^= n >>> 13
n = Math.imul(n, 3266489909) >>> 0
n ^= n >>> 16`,
    note: '해시를 난수 대신 쓸 때는 섞는 단계가 필요했다. 비슷한 문자열을 넣으면 결과도 비슷해진다는 걸 몰랐다.',
  },
  {
    id: 't12',
    title: '캐릭터가 제자리에서 20px 만 오감',
    date: '내 사람들 마당',
    symptom:
      '마당을 가로질러 걷게 했는데 아무리 기다려도 제자리에서 조금씩 움직이기만 했다. 마당 폭은 860px 인데 실제로는 20px 남짓만 오갔다.',
    log: `@keyframes cross {
...
// 실제 x 좌표를 재봤다
처음 위치: [453, 0, 538]
3초 뒤:   [449, 0, 540]   /* 거의 그대로 */`,
    cause:
      'transform 의 100% 는 부모 폭이 아니라 자기 자신의 폭이다. 캐릭터는 20px 남짓이라 100% 도 20px 이었다. width 나 left 에서의 100% 와 기준이 다르다.',
    fix: `/* 캐릭터를 감싼 칸의 폭을 마당 전체로 잡는다 */
.walker {
  width: 100%;   /* 여기서의 100% 는 부모 폭 */
}
/* 이제 translateX(100%) 가 마당 폭만큼이 된다 */`,
    note: '같은 100% 인데 속성마다 기준이 다르다. 값이 안 먹는 게 아니라 내가 생각한 기준이 아니었다.',
  },
  {
    id: 't11',
...
const reviewList = ref([
  {
    id: 'r18',
    title: '화살표만 두었더니 누를 수 있는 줄 몰랐던 것',
    when: '내 사람들 마당',
    why: '배경 고르는 버튼을 마당 안에 이름 다섯 개로 늘어놓았더니 휴대폰에서 마당 폭의 절반을 넘게 차지해 무대를 가렸다. 밖으로 빼고 화살표만 남겼는데, 이번엔 그게 누를 수 있는 것인지도, 바꿀 게 몇 개인지도 알 수 없었다. 기능이 있는 줄 모르면 없는 것과 같다.',
    before: `<!-- 마당 안, 이름 다섯 개 -->
<div class="picker">
  <button>들판</button><button>바닷가</button><button>밤하늘</button>
  <button>눈밭</button><button>골목</button>
</div>
/* 휴대폰에서 재보니 */
배경버튼 187px / 마당 268px`,
    after: `<!-- 마당 밖, 테두리 두른 화살표 + 점 다섯 -->
( ‹ )   ● ○ ○ ○ ○   ( › )

.arrow { border: 1px solid var(--color-line); border-radius: 50%; }
.dot.on { background: var(--color-ink); transform: scale(1.35); }`,
    note: '점은 개수와 지금 위치를 한 번에 알려 주면서 이름을 늘어놓지 않아도 된다. 눌러서 바로 그 배경으로 갈 수도 있다. 테두리 하나 두르는 것만으로 "눌러도 되는 것" 이 됐다.',
  },
  {
    id: 'r17',
    title: '들어오자마자 위치를 묻던 것',
    when: '지금 있는 곳의 날씨',
    why: '위치 날씨를 붙이면서 처음에는 화면이 뜨자마자 좌표를 요청했다. 그런데 무슨 사이트인지 알기도 전에 권한 팝업이 뜨면 일단 거절하게 되고, 한 번 거절하면 브라우저 설정을 열기 전까지 되돌릴 수 없다.',
    before: `onMounted(() => {
  navigator.geolocation.getCurrentPosition(...)   // 바로 팝업
})`,
    after: `// 이미 허용해 둔 경우에만 조용히 불러온다.
// Permissions API 는 팝업을 띄우지 않고 지금 상태만 알려준다.
const permission = await navigator.permissions?.query({ name: 'geolocation' })
if (permission?.state === 'granted') load()

// 아직 안 물어본 사람에게는 누를 수 있는 글자 하나로 둔다`,
    note: "거절(code 1)과 그 밖의 실패를 나눴다. 거절은 다시 조르지 않고 물러나고, 시간 초과처럼 다시 될 수 있는 실패만 '다시' 로 바꿔 재시도를 열어 뒀다. 권한은 한 번 닫히면 내가 열 수 없다.",
  },
  {
    id: 'r16',
